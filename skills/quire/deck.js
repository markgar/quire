 ...
 *
 * @param {string} text
 * @returns {{docMeta: Record<string, string>, chunks: string[][], chunkLines: number[]}}
 */
export function splitSlides(text) {
  ...
  for (const [relativeIndex, line] of lines.slice(start).entries()) {
    ...
    if (!inFence && SEP.test(line)) {
      ...
      cur = [];
      chunkLine = start + relativeIndex + 2;
    } else {
      cur.push(line);
    }
  ...
  const populated = chunks
    .map((chunk, index) => ({ chunk, line: chunkLines[index] }))
    .filter(({ chunk }) => chunk.some((line) => strip(line)));
  return {
    docMeta,
    chunks: populated.map(({ chunk }) => chunk),
    chunkLines: populated.map(({ line }) => line),
  };
}
...
export function parseQuire(text, opts = {}) {
  const { docMeta, chunks, chunkLines } = splitSlides(text);
  const slides = chunks.map((chunk, index) => {
    try {
      return buildSlide(chunk, index === 0, opts);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(`slide ${index + 1}, line ${chunkLines[index]}: ${message}`);
    }
  });
