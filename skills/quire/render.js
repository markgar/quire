...
/**
 * Image sources are deliberately limited to same-origin paths and base64 image
 * data. Raw HTML remains the explicit escape hatch for anything else.
 *
...
export function imageSource(value) {
  ...
  if (/^data:image\/(?:png|jpe?g|gif|webp|avif|svg\+xml);base64,[a-z0-9+/=\s]+$/i.test(source)) return source;
