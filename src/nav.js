// @ts-check
/* global fitMetricValuesAfterFonts */
...
(() => {
  ...
  const prev = /** @type {HTMLButtonElement} */ (document.getElementById("prev"));
  ...
  const dotWrap = /** @type {HTMLElement} */ (document.getElementById("dots"));
  const burger = /** @type {HTMLButtonElement} */ (document.getElementById("burger"));
  ...
  function sizeThumbs() {
    panelList.querySelectorAll(".thumb-frame").forEach(frame => {
      const f = /** @type {HTMLElement} */ (frame);
      const s = f.clientWidth / SLIDE_W;
      const inner = /** @type {HTMLElement} */ (f.querySelector(".thumb-scale"));
  ...
  function render() {
    ...
    buildDots();
    Array.from(dotWrap.children).forEach(dot => {
      const d = /** @type {HTMLElement} */ (dot);
      d.classList.toggle("on", +String(d.dataset.index) === i);
    });
    ...
    panelList.querySelectorAll(".thumb").forEach(thumb => {
      ...
      const num = /** @type {HTMLElement} */ (t.querySelector(".thumb-num"));
      const toggle = /** @type {HTMLButtonElement} */ (t.querySelector(".thumb-toggle"));
      num.textContent =
        hidden[n] ? "Hidden" : "Slide " + (shownIndexes().indexOf(n) + 1);
      toggle.textContent = hidden[n] ? "Show" : "Hide";
      ...
      const flag = /** @type {HTMLElement} */ (t.querySelector(".thumb-over"));
      if (over) {
  ...
  /** @param {number | undefined} [keepIndex] */
